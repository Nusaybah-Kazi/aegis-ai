...
export const evaluateAction = (data) => api.post('/gateway/evaluate', {
  ...
  tool_name:  data.tool_name ?? data.tool_id,   // use tool_name if provided, fall back to tool_id
  action:     data.action ?? data.action_type,
  parameters: JSON.stringify(data.payload ?? {}),
})

// Backend expects: { reviewed_by, reason }
export const approveAction = (id, note) => api.post(`/gateway/approve/${id}`, {
  reviewed_by: 'human-reviewer',
  reason:      note || 'Approved via Aegis UI',
})
export const denyAction = (id, note) => api.post(`/gateway/deny/${id}`, {
  reviewed_by: 'human-reviewer',
